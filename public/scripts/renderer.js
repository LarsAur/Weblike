class Renderer {
    ...
    draw_character(character, dir, anim, x, y) {
        // Character sprite sheet is direction along x-axis and animation stage along y-axis
        // drawImage(img, start_clipx, start_clipy, w_clip, h_clip, x_pos, y_pos, draw_w, draw_h)
        if (dir) {
            this.ctx.drawImage(character, this.direction_to_animation.get(dir) * 16, anim * 16, 16, 16, x, y, 16, 16)
        } else {
            this.ctx.drawImage(character, 0, anim * 16, 16, 16, x, y, 16, 16)
        }
    }
    ...
    draw_fog() {
        let grd = this.ctx.createRadialGradient(150, 125, 100, 150, 125, 140)
